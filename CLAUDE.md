...
- `/dashboard/*`는 이메일 **등록**까지만 강제(proxy에서 `/onboarding/email`로 리다이렉트).
  인증 완료는 강제하지 않는다.
- 카카오가 이메일을 주는 계정은 온보딩을 건너뛰고 바로 들어온다. 그 주소를 바꾸려면
  `/onboarding/email?mode=change` — **변경 모드**가 없으면 `user.email`이 있다는 이유로
  즉시 되돌려보내 변경 자체가 불가능했다. 계정 설정의 '이메일 변경'이 이 링크를 쓴다.
  변경은 새 주소의 인증 링크를 눌러야 완료되고, 그때까지는 기존 주소로 메일이 간다.
...
- **이메일 변경은 Secure email change(기본 활성) 때문에 양쪽 확인이 필요하다.**
  새 주소만 누르면 `auth.users.new_email`에 남아 변경이 완료되지 않는다. 대시보드
  Authentication → Email에서 이 옵션을 끄면 새 주소 1회 클릭으로 끝난다.
  옵션을 켠 채로 두려면 계정 설정의 '변경 대기 중' 안내(양쪽 링크 필요)가 그 역할을 한다.
  → 계정 설정은 `user.new_email`이 있으면 항상 대기 상태를 표시한다. 표시하지 않으면
  사용자는 옛 주소 + '인증 완료' 배지만 보고 "변경이 씹혔다"고 판단한다.
