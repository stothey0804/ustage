...
export function BookingForm({
  ...
  if (!isOpen) {
    // 카드·버튼 규격은 열린 상태(1단계 CTA)와 같게 맞춘다 — 같은 자리에 같은 쌍의
    // 버튼이 놓이므로 크기가 다르면 마감 화면만 어색해 보인다.
    return (
      <div className="space-y-4 rounded-4xl bg-card p-5 text-center shadow-md ring-1 ring-foreground/5">
          ...
          {isLoggedIn ? (
            ...
            <div className="flex gap-3">
              ...
              <Button asChild size="lg" variant="outline" className="flex-1">
                <Link href={`${pathname}/me`}>비회원 조회</Link>
              </Button>
            </div>
          )}
        </div>
