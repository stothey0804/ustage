...
price           integer          # 원 단위
onsite_price    integer nullable # 현장 예매 1매 가격 — NULL이면 price와 동일
...
- **상태 전환은 조건부 갱신**(`.neq("status", 목표상태)` + `select`)으로 처리한다 —
  단건·일괄 모두. 소유자와 스태프가 동시에 같은 건을 확인해도 실제로 바뀐 쪽만
  1건으로 세고 메일도 그 건에만 나간다. 이미 같은 상태였던 요청은 `updated: 0`으로
  돌아와 화면이 "이미 확정된 예매입니다" 같은 안내 토스트를 띄운다.
...
진입점은 **일부러 눈에 띄지 않는 회색 밑줄 텍스트 링크**다(버튼 아님) — 삭제되는
데이터와 차단 조건 고지는 확인 다이얼로그가 담당한다.

- **차단 조건 1**: 내가 만든 스테이지에 예매 이력(취소분 포함)이 하나라도 있으면 탈퇴 불가.
  주최자가 사라지면 입금 확인·입장 처리를 할 사람이 없어지므로 먼저 정리하게 안내한다.
- **차단 조건 2**: 참석자로서 **입장 예정인 예매**(취소되지 않은 예매 중
  `event_end_date ?? event_date` 미경과)가 있으면 탈퇴 불가. 탈퇴하면 예약 조회
  수단이 사라져 입장 QR·안내를 받을 수 없기 때문. 스테이지가 종료된 예매는 막지 않는다.
...
- **Resend 무료 플랜은 일 100통·월 3,000통 상한이다.** 입금확인 일괄 처리(50명=50통)와 가입
  인증이 겹치면 막힌다. AWS SES 이관 검토를 마쳤고(하드 블로커 없음, 게이트는 프로덕션 승인
  하나) 계획을 `docs/plans/ses-migration.md`에 남겼다 — **미착수**. 급하면 Resend Pro($20/월,
  이관 비용 0)가 즉효 대안이다.
...
- **참석자용 두 화면(회원 예약 상세 · 비회원 조회)은 헤더 구성을 통일한다** — 왼쪽 이름,
  오른쪽에 `BookingStatusBadge` + 매수 배지(+부분 취소 배지), 그 아래 예매번호를
  `font-mono text-primary`로. 비회원 조회에 있던 자체 상태 라벨 구현은 걷어냈다
  (라벨이 갈라지면 같은 예약이 화면마다 다르게 보인다).
...
- 홈(`/dashboard`)은 목록이 아니라 **요약**: 인사 한 줄(D-day·입금대기 건수) +
  ...
  주최 이력이 없으면(`isHost = events.length > 0`) 생성 유도 카드가 아니라 내 티켓과
  같은 **빈 상태 문구만** 보여준다("내가 만든 스테이지가 없어요" — CTA 버튼 없음).
  "다가오는" 판정은 `daysUntil`(24시간 올림)이 아니라 **시각 기준**
  (`event_end_date ?? event_date` 미경과)이다 — 올림 판정은 종료 후 하루까지
  "공연이 오늘이에요"로 보이는 버그가 있었다.
- **로그아웃은 계정 설정(`/dashboard/account`)에만 있다** — 헤더에는 계정 진입점만 남긴다.
